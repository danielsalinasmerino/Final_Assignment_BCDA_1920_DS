...
contract('Asignatura:', accounts => {
    ...
    console.log('Fichero de Pruebas modificado por Daniel Salinas Merino')

    // Pruebas creadas por Daniel Salinas Merino
    it("NUEVO: Un alumno consulta su nota, pero con valores incorrectos (usaremos assert.notEqual)",  async () => {

        let pepeAccount = accounts[2];

        let nota = await asignatura.miNota(1, {from: pepeAccount});

        assert.notEqual(nota.tipo, 2, "El alumno no puede recuperar su tipo de nota.");
        assert.notEqual(nota.calificacion, 40, "El alumno no puede recuperar su calificacion.");
    })

    it("NUEVO: Test que debe fallar porque intentamos matricularnos sin nombre",  async () => {

        let ramiroAccount = accounts[3];

        await asignatura.automatricula("", "ramiro_gomez@gmail.com", {from: ramiroAccount});
    })

    it("NUEVO: Test que debe fallar porque intentamos preguntar quién somos sin estar matriculados",  async () => {
        
        let pepeAccount = accounts[3];

        await asignatura.quienSoy({from: pepeAccount});
    })

    it("NUEVO: Test que debe fallar porque intentamos matricularnos con una cuenta YA matriculada",  async () => {
